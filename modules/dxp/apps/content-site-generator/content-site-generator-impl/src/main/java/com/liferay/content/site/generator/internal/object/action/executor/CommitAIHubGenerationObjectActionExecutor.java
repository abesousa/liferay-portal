...
public class CommitAIHubGenerationObjectActionExecutor
	...
	private String _awaitCompletion(String fileName, long taskId)
	...
	private void _completeExceptionally(
			String failureReason, long objectEntryId, long userId)
	...
	private void _doExecute(long companyId, long objectEntryId, long userId) {
		try {
				...
				_objectEntryLocalService.getValuesList(
					...
					_filterFactory.create(
						StringBundler.concat(
							...
							"'"),
						objectDefinition),
					null, -1, -1, new Sort[] {new Sort("loadOrder", false)});
			...
			for (Map<String, Serializable> values : valuesList) {
				...
				long fileEntryId = GetterUtil.getLong(values.get("batchFile"));

				BatchEngineImportTask batchEngineImportTask =
					_executeBatchEngineImportTask(
						companyId, fileEntryId, fileName, userId);
				...
				if (errorMessage != null) {
					_completeExceptionally(errorMessage, objectEntryId, userId);
			...
			try {
				_completeExceptionally(
					exception1.getMessage(), objectEntryId, userId);
			}
	...
	private BatchEngineImportTask _executeBatchEngineImportTask(
		...
		try (InputStream inputStream = fileEntry.getContentStream()) {
			...
			if (Validator.isBlank(batchFileJSON)) {
				throw new InvalidFileException(
					"Empty batch file: " + fileEntry.getFileName());
			}
		}
