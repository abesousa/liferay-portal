...
import {GAP_INSIGHTS_AGENT} from './types';
...
export async function createGapInsightsEventSource(): Promise<EventSource | null> {
...
export async function postGapInsightsAgentInstance({
	context,
	sseEventSinkKey,
}: {
	...
	const response = await fetch(
		...
		{
			body: JSON.stringify({
				agentDefinitionExternalReferenceCode: GAP_INSIGHTS_AGENT,
				context,
				sseEventSinkKey,
			}),
			headers: new Headers({
